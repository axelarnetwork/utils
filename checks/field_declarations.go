...
func getRequiredFieldNames(s *types.Struct, deprecatedFields map[token.Pos]struct{}, currentPackage string, exclusionPatterns ...string) []string {
	requiredFields := slices.Filter(getAllNotIgnoredFields(s), isRequired(deprecatedFields, currentPackage, exclusionPatterns...))
	return slices.Map(requiredFields, (*types.Var).Name)
}

func getAllNotIgnoredFields(s *types.Struct) []*types.Var {
	var fields []*types.Var
	for i := 0; i < s.NumFields(); i++ {
		field := s.Field(i)
		tag := s.Tag(i)
		if !shouldBeIgnored(tag) {
...
func shouldBeIgnored(tag string) bool {
	return regexp.MustCompile(`decl-check:"ignore"`).MatchString(tag)
}
